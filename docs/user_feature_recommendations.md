...
### 📧 Email Delivery — "Your Briefing, In Your Inbox"

Send the daily voice briefing straight to the user's email. The email includes a **written summary** of each story (scannable in 30 seconds) plus an **attached or linked audio file** so they can listen without opening the app at all.

#### Why This Matters
- **Meet users where they already are** — most people check email before any app; the briefing lands in their morning routine automatically
- **Offline access** — an attached MP3 can be downloaded and played on a plane, subway, or anywhere without data
- **Shareability** — users can forward the email to friends/colleagues ("you should subscribe to this") — organic growth channel
- **Reduced app-open friction** — even if a user skips the app for a day, they still get value → prevents churn

#### What the User Sees

```
Settings → Daily Briefing → Delivery Method:
  → ☑ In-App Player (default, always on)
  → ☐ Email My Briefing
      → Email: rishabh@example.com (pre-filled from account)
      → Schedule: 7:00 AM / 8:00 AM / 9:00 AM / Custom
      → Format: "Summary + Audio Link" | "Summary + Attached MP3"

Email Received (7:00 AM):
  ┌──────────────────────────────────────┐
  │ 📰 Your Daily Voice Briefing        │
  │    Friday, July 11, 2026            │
  │                                      │
  │ 🎧 Listen to your briefing:         │
  │    [▶ Play Audio] (link to hosted   │
  │     MP3 or inline audio player)     │
  │                                      │
  │ ── Today's Stories ──────────────── │
  │                                      │
  │ 1. 🔬 Science                       │
  │    NASA's Mars rover discovers...   │
  │    → Read full article              │
  │                                      │
  │ 2. 💻 Technology                    │
  ...
  │                                      │
  │ 3. 📈 Finance                       │
  │    Markets rally as Fed signals...  │
  │    → Read full article              │
  │                                      │
  │ ... 2 more stories                  │
  │                                      │
  │ ──────────────────────────────────── │
  │ 🔇 Unsubscribe | ⚙️ Change prefs   │
  └──────────────────────────────────────┘
```
...
| Component | Detail |
|---|---|
| **Email service** | Integrate with Nodemailer + an SMTP provider (e.g., SendGrid, AWS SES, or Resend) for transactional email |
| **Email template** | Responsive HTML email template with story summaries, article links, and audio player/download |
| **Audio hosting** | Store generated TTS audio (MP3) in cloud storage (S3/GCS) with a short-lived signed URL, or attach directly for the MP3 option |
| **Delivery scheduler** | Cron job triggered at user's chosen delivery time (timezone-aware) — can piggyback on the existing briefing generation scheduler |
| **Preferences model** | Extend user settings: `{ emailBriefing: true, emailTime: "07:00", emailFormat: "link" \| "attachment", timezone: "Asia/Kolkata" }` |
| **Unsubscribe handler** | One-click unsubscribe link in every email (required by CAN-SPAM / GDPR) with `GET /api/briefing/unsubscribe?token=...` |
| **Analytics** | Track email open rates, audio play-through rates, and click-through to articles |
...
- 📧 **Inbox delivery**: Reaches users who don't open the app daily — reduces churn and drives re-engagement
- 📤 **Organic growth**: Forwarded briefing emails become a natural referral channel
...
| 🥉 P3 | **Daily Voice Briefing + Email** | "I want news while multitasking" | ⭐⭐⭐⭐⭐ | Medium-High |
